...
// Shadow leggero per leggibilità
const _kTextShadow = [
...
class _WeeklyCalendarState extends State<WeeklyCalendar> {
  ...
  // Subscription rooms (real-time ha senso solo per stanze)
  StreamSubscription? _roomsSub;

  // Cache layout calcolato per evitare ricalcoli inutili
  List<Appointment> _lastApts = [];
  Map<String, Map<String, _SlotLayout>> _layoutCache = {}; // key = day ISO
  ...
  // Carica utenti e clienti una sola volta (dati raramente aggiornati)
  Future<void> _loadStaticData() async {
    ...
    for (final doc in usersSnap.docs) {
      ...
      n[doc.id] = d['displayName']?.toString() ??
          d['email']?.toString() ?? 'Utente';
    }

    final cl = <String, String>{};
    for (final doc in clientsSnap.docs) {
      cl[doc.id] =
          '${doc.data()['nome'] ?? ''} ${doc.data()['cognome'] ?? ''}'.trim();
    }
    ...
    // Stanze in real-time (cambiano raramente, ma vogliamo i colori aggiornati)
  ...
  Color _rColor(String? id) {
    ...
    try {
      return Color(
          int.parse('FF${_rooms[id]!.color.replaceAll('#', '')}', radix: 16));
    } catch (_) {
      return Colors.grey;
    }
  }
  ...
  // Layout calcolato e memorizzato per giorno; si ricalcola solo se _lastApts cambia
  Map<String, _SlotLayout> _layoutForDay(String dayKey, List<Appointment> apts) {
    if (!_layoutCache.containsKey(dayKey)) {
      _layoutCache[dayKey] = _computeLayout(apts);
    }
    return _layoutCache[dayKey]!;
  }
  ...
  Map<String, _SlotLayout> _computeLayout(List<Appointment> apts) {
    ...
    for (final cluster in clusters) {
      ...
      if (n <= _kMaxVisible) {
        ...
        result[
            '__badge__${cluster.first.id ?? cluster.first.titolo}'] = _SlotLayout(
          colIndex: _kMaxVisible - 1,
          colCount: _kMaxVisible,
          isOverflow: true,
          overflowCount: overflow.length,
          overflowApts: overflow,
          clusterOraInizio: cluster
              .map((a) => a.oraInizio)
              .reduce((a, b) => _toMin(a) < _toMin(b) ? a : b),
          clusterOraFine: cluster
              .map((a) => a.oraFine)
              .reduce((a, b) => _toMin(a) > _toMin(b) ? a : b),
        );
      }
  ...
  List<Appointment> _applyFilters(List<Appointment> apts) {
    if (widget.filterUserId == null &&
        widget.filterRoomId == null &&
        widget.filterClientId == null) return apts;
    return apts.where((a) {
      if (widget.filterUserId != null) {
        if (a.userId != widget.filterUserId &&
            !a.workerIds.contains(widget.filterUserId)) return false;
      }
      if (widget.filterRoomId != null && a.roomId != widget.filterRoomId)
        return false;
      if (widget.filterClientId != null && a.clientId != widget.filterClientId)
        return false;
      return true;
    }).toList();
  }
  ...
  Widget _aptContent({
    ...
    return OverflowBox(
      ...
      child: Padding(
        padding: EdgeInsets.fromLTRB(isNarrow ? 4 : 6, 3, isNarrow ? 3 : 5, 3),
        child: isShort
            ? Row(
                ...
                children: [
                  ...
                  if (hasPaziente) ...[
                    ...
                    Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(Icons.person, size: isNarrow ? 8 : 9, color: Colors.black54),
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                        isNarrow ? apt.oraInizio : '${apt.oraInizio}-${apt.oraFine}',
                        style: TextStyle(
                            fontSize: 9,
                            fontWeight: FontWeight.w600,
                      ...
                      if (canSee) ...[
                        ...
                        if (apt.note != null && apt.note!.trim().isNotEmpty)
                          Padding(
                            ...
                            child: Icon(Icons.sticky_note_2, size: 9, color: Colors.amber.shade800),
                          ),
                        Icon(
                          apt.fatturato ? Icons.receipt_long : Icons.receipt_long_outlined,
                          size: 9,
                          color: apt.fatturato ? Colors.orange.shade800 : Colors.black38,
                        ),
                        const SizedBox(width: 2),
                        Icon(
                          apt.pagato ? Icons.check_circle : Icons.radio_button_unchecked,
                          size: 9,
                          color: apt.pagato ? Colors.green.shade700 : Colors.black38,
                        ),
                      ],
                    ],
                  ),
  ...
  Widget build(BuildContext context) {
    ...
    // Calcola i giorni della settimana una volta sola
    final days = _days;
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return StreamBuilder<List<Appointment>>(
          stream: _aptService.getAppointments(days.first, days.last),
          builder: (context, snap) {
            final allApts = snap.data ?? [];

            // Invalida cache solo quando gli appuntamenti cambiano davvero
            if (allApts != _lastApts) {
              _lastApts = allApts;
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        ...days.map((d) {
                          ...
                          final layout = _layoutForDay(dayKey, dayApts);
                                  ...
                                  children: [
                                    ...
                                    ...dayApts.map((apt) {
                                      ...
                                      return Positioned(
                                        ...
                                        width: colW,
                                        child: _AptCard(
                                          key: ValueKey(apt.id ?? apt.titolo),
                                          apt: apt,
                                          cardH: cardH,
  ...
  Widget _buildNowLine() {
    ...
    return Positioned(
      ...
      child: Row(children: [
        ...
        Expanded(
            child: Container(height: 1.5, color: Colors.redAccent.withOpacity(0.6))),
      ]),
    );
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Widget estratti per evitare rebuild inutili con RepaintBoundary
// ─────────────────────────────────────────────────────────────────────────────

/// Cella griglia (sfondo + tap zone) — const-friendly, non ricostruisce mai
class _GridCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: isToday ? primaryColor.withOpacity(0.02) : Colors.white,
          border: Border(
...
/// Card appuntamento estratta come StatelessWidget con key → Flutter può
/// riusare l'elemento invece di ricostruirlo ad ogni rebuild del calendario.
...
/// Card nel bottom sheet overflow
class _SheetCard extends StatelessWidget {
  final Appointment apt;
  final bool canSee;
  final String clienteNome;
  final List<Color> workerColors;
  final Color rColor;
  final Room? room;
  final VoidCallback onTap;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: ClipRRect(
        ...
        child: CustomPaint(
          painter: _StripePainter(colors: workerColors, stripeHeight: 10.0, opacity: 0.20),
          child: Stack(
            children: [
              Positioned(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(apt.titolo,
                              ...
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis),
                          if (canSee && clienteNome.isNotEmpty && clienteNome != apt.titolo) ...[
                            const SizedBox(height: 2),
                            Row(children: [
                              const Icon(Icons.person_outline, size: 11, color: Colors.black45),
                              const SizedBox(width: 3),
                              Expanded(
                          ...
                          Row(children: [
                            const Icon(Icons.access_time, size: 11, color: Colors.black45),
                            const SizedBox(width: 3),
